...
import {
  getUserBMIBySessionId,
  getUserCalendarBySessionId,
  TrainingPlanType,
...
export default function Dashboard({
  ...
  data: Ship;
  plan: string[];
  calenD: HighlightedDay[];
  today: TrainingPlanType;
  bmi: number; // Type for the BMI
}) {
          ...
          <div className="content-area">
            <div className="training-program">
              <div className="program-content">
                {selectedOption === "Training Plan" && (
                  <TrainingPlan plan={today} />
                )}
                {selectedOption === "Diet Plan" && <DietPlan bmi={bmi} />}
              </div>
            </div>
          </div>
        </main>
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  if (cookies) {
    ...
    if (sessionID) {
      ...
      const { bmi } = await getUserBMIBySessionId(sessionID);
